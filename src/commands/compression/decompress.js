...
const decompressFile = async (pathToFileZip, pathToFileDecompressed) => {
  try {
    const isFileExists = await access(pathToFileZip, constants.F_OK).then(() => true).catch(() => {
      console.log(`The file to be decompressed does not exist: "${pathToFileZip}"`);
      return false;
    });

    const isDecompressedFileExists = await access(pathToFileDecompressed, constants.F_OK).then(() => {
      console.log(`The decompressed file already exists in this directory: "${pathToFileDecompressed}"`);
      return true;
    }).catch(() => false);
    ...
    if (isFileExists && !isDecompressedFileExists) {
      ...
      console.log(`File has been successfully decompressed and saved to: "${pathToFileDecompressed}"`);
    }
  } catch (error) {
