...
const compressFile = async (pathToFile, pathToFileCompressed) => {
  try {
    const isFileExists = await access(pathToFile, constants.F_OK).then(() => true).catch(() => {
      console.log(`The file to be compressed does not exist: "${pathToFile}"`);
      return false;
    });

    const isZipExists = await access(pathToFileCompressed, constants.F_OK).then(() => {
      console.log(`The compressed file already exists in this directory: "${pathToFileCompressed}"`);
      return true;
    }).catch(() => false);
    ...
    if (isFileExists && !isZipExists) {
      ...
      console.log(`File has been successfully compressed and saved to: "${pathToFileCompressed}"`);
    }
  } catch (error) {
